<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-xl text-slate-900 dark:text-slate-100 leading-tight">Edit Metal: {{ $metal->name }}</h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('admin.metals.update', $metal) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-100 focus:ring-amber-500 focus:border-amber-500 transition-colors">
                            <option value="active" {{ old('status', $metal->status) == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="inactive" {{ old('status', $metal->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <template x-for="(p, index) in purities" :key="index">
                            <div class="grid grid-cols-12 gap-2 mb-3 items-end">
                                ...
                                <div class="col-span-2">
                                    <label class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Status</label>
                        ...
                        <p x-show="purities.length === 0" class="text-xs text-slate-500 italic">No purities. Click + Add Purity.</p>
                    </div>
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        ...
                        <x-primary-button>Update Metal</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
