<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-xl text-slate-900 dark:text-slate-100 leading-tight">{{ __('Add New Metal') }}</h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl transition-colors" x-data="metalForm()">
                <form method="POST" action="{{ route('admin.metals.store') }}" class="space-y-6">
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <p x-show="purities.length === 0" class="text-xs text-slate-500 italic">{{ __('No purities added yet.') }}</p>
                    </div>
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        ...
                        <x-primary-button>{{ __('Save Metal') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
