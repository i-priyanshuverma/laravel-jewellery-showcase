<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-xl text-slate-900 dark:text-slate-100 leading-tight">Add New Metal</h2>
    </x-slot>
    ...
    <div class="py-8 bg-slate-50 dark:bg-slate-950 min-h-screen text-slate-900 dark:text-slate-100 transition-colors duration-200">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl transition-colors" x-data="metalForm()">
                <form method="POST" action="{{ route('admin.metals.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="col-span-2">
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required autofocus placeholder="e.g. Gold" />
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">Purities</h3>
                            <button type="button" @click="addPurity()" class="text-xs font-bold text-amber-600 dark:text-amber-400 hover:underline">+ Add Purity</button>
                        </div>

                        <template x-for="(p, index) in purities" :key="index">
                            <div class="grid grid-cols-12 gap-2 mb-3 items-end">
                                <div class="col-span-5">
                                    <label class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Display Name</label>
                                ...
                                <div class="col-span-3">
                                    <label class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Value</label>
                                ...
                                <div class="col-span-2">
                                    <label class="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-semibold">Sort</label>
                                ...
                                <div class="col-span-2 text-right">
                                    <button type="button" @click="purities.splice(index, 1)" class="text-xs text-rose-600 dark:text-rose-400 hover:underline font-semibold">Remove</button>
                                </div>
                            </div>
                        </template>

                        <p x-show="purities.length === 0" class="text-xs text-slate-500 italic">No purities added yet.</p>
                    </div>

                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <a href="{{ route('admin.metals.index') }}" class="text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200">Cancel</a>
                        <x-primary-button>Save Metal</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        function metalForm() {
            return {
                purities: [],
                addPurity() {
                    this.purities.push({ name: '', value: '', sort_order: this.purities.length + 1 });
                }
            };
        }
    </script>
