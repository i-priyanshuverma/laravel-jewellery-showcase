<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.categories.create') }}" class="px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-sm transition">+ Add New Category</a>
        </div>
    </x-slot>
                ...
                <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                    <thead class="bg-slate-50 dark:bg-slate-950/60 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-3.5">Total Products</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse ($categories as $category)
                            <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 rounded-full bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-amber-700 dark:text-amber-400 text-xs font-semibold">{{ $category->products_count }} products</span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <x-table-action-group :edit-url="route('admin.categories.edit', $category)"
                                                          ...
                                                          delete-confirm="Delete category {{ $category->name }}?" />
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-8 text-center text-slate-500 dark:text-slate-400">No categories found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
