<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('vendor.products.create') }}" class="px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-sm transition">+ Add Product</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse ($products as $product)
                            <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/50 transition">
                                ...
                                <td class="px-6 py-4 text-right space-x-3">
                                    <a href="{{ route('vendor.products.show', $product) }}" class="text-xs font-bold text-amber-600 dark:text-amber-400 hover:underline">Manage</a>
                                    <a href="{{ route('vendor.products.edit', $product) }}" class="text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white">Edit</a>
                                    <form method="POST" action="{{ route('vendor.products.destroy', $product) }}" class="inline" onsubmit="event.preventDefault(); window.confirmAction({ title: 'Delete Product', message: 'Are you sure you want to delete &quot;{{ addslashes($product->name) }}&quot;? All associated variants and images will be permanently removed, and active reservations will be released.', confirmText: 'Delete Product', icon: 'danger', form: this });">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">No products found. <a href="{{ route('vendor.products.create') }}" class="text-amber-600 dark:text-amber-400 font-semibold underline">Create your first product</a></td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
