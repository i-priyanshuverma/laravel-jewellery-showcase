<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('vendor.imports.create') }}" class="px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-sm transition">{{ __('+ Upload New CSV') }}</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                    <thead class="bg-slate-50 dark:bg-slate-950/60 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        <tr>
                            <th class="px-6 py-3.5">{{ __('File Name') }}</th>
                            <th class="px-6 py-3.5">{{ __('Total Rows') }}</th>
                            <th class="px-6 py-3.5">{{ __('Processed') }}</th>
                            <th class="px-6 py-3.5">{{ __('Success / Fail') }}</th>
                            <th class="px-6 py-3.5">{{ __('Status') }}</th>
                            <th class="px-6 py-3.5">{{ __('Date Uploaded') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse ($imports as $import)
                            <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/50 transition">
                                ...
                                <td class="px-6 py-4 font-semibold">{{ $import->total_rows ?? __('Counting...') }}</td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('vendor.imports.show', $import) }}" class="text-xs font-bold text-amber-600 dark:text-amber-400 hover:underline">{{ __('View Progress & Results →') }}</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">{{ __('No CSV imports uploaded yet.') }} <a href="{{ route('vendor.imports.create') }}" class="text-amber-600 dark:text-amber-400 font-semibold underline">{{ __('Upload your first CSV file') }}</a></td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
