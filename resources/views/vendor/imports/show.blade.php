    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-extrabold text-xl text-slate-900 dark:text-slate-100 leading-tight">
                {{ __('Import Status:') }} {{ $import->filename }}
            </h2>
            <a href="{{ route('vendor.imports.index') }}" class="text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200">&larr; {{ __('Back to Imports List') }}</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 transition-colors">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                            <span>{{ __('CSV Processing Dashboard') }}</span>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ __('Uploaded on') }} {{ $import->created_at->format('M d, Y \a\t H:i') }}</p>
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                    <div class="p-3.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                        <span class="text-xs text-slate-500 dark:text-slate-400 font-medium">{{ __('Total Rows') }}</span>
                        <div class="text-lg font-bold text-slate-900 dark:text-white" x-text="totalRows">{{ $import->total_rows ?? '...' }}</div>
                    </div>
                    <div class="p-3.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                        <span class="text-xs text-slate-500 dark:text-slate-400 font-medium">{{ __('Processed') }}</span>
                    ...
                    <div class="p-3.5 bg-emerald-50 dark:bg-emerald-950/40 rounded-xl border border-emerald-200 dark:border-emerald-800">
                        <span class="text-xs text-emerald-700 dark:text-emerald-400 font-bold">{{ __('Successful Records') }}</span>
                    ...
                    <div class="p-3.5 bg-rose-50 dark:bg-rose-950/40 rounded-xl border border-rose-200 dark:border-rose-800">
                        <span class="text-xs text-rose-700 dark:text-rose-400 font-bold">{{ __('Failed Records') }}</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 transition-colors">
                    <h3 class="text-base font-bold text-rose-600 dark:text-rose-400 flex items-center justify-between">
                        <span>{{ __('Failed Records') }} ({{ $import->failed_rows }})</span>
                        <span class="text-xs text-slate-400 font-normal">{{ __('Validation / Parsing Errors') }}</span>
                    </h3>
                    ...
                    <div class="space-y-3">
                        @forelse ($failedRows as $failed)
                            <div class="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/50 text-xs">
                                <div class="flex items-center justify-between font-bold text-rose-900 dark:text-rose-200">
                                    <span>{{ __('Row #') }}{{ $failed->row_number }}</span>
                        ...
                        @empty
                            <p class="text-sm text-slate-500 dark:text-slate-400 py-4">{{ __('No failed records.') }}</p>
                        @endforelse
                    </div>
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 transition-colors">
                    <h3 class="text-base font-bold text-emerald-600 dark:text-emerald-400 flex items-center justify-between">
                        <span>{{ __('Successfully Imported') }} ({{ $import->successful_rows }})</span>
                        <span class="text-xs text-slate-400 font-normal">{{ __('Inserted / Updated') }}</span>
                    </h3>
                    ...
                    <div class="space-y-3">
                        @forelse ($successRows as $success)
                            <div class="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50 text-xs flex items-center justify-between">
                                ...
                                <span class="text-emerald-600 dark:text-emerald-400 font-bold">✓ {{ __('Success') }}</span>
                            </div>
                        @empty
                            <p class="text-sm text-slate-500 dark:text-slate-400 py-4">{{ __('No successful records yet.') }}</p>
                        @endforelse
                    </div>
