<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-slate-600 dark:text-slate-400 px-3 py-1 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-full">
                {{ __('Category:') }} <strong class="text-amber-700 dark:text-amber-400">{{ $product->category?->name ?? __('Uncategorized') }}</strong>
            </span>
        </div>
    </x-slot>
    ...
    <div class="py-8 bg-slate-50 dark:bg-slate-950 min-h-screen text-slate-900 dark:text-slate-100 transition-colors duration-200" x-data="variantCreateForm()">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-2xl space-y-8 transition-colors">
                ...
                <form method="POST" action="{{ route('vendor.products.variants.store', $product) }}" class="space-y-8">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <x-text-input id="sku" name="sku" type="text" class="mt-1 block w-full uppercase font-mono" :value="old('sku')" required placeholder="e.g. DSR-18K-YG-6" autofocus />
                                <x-input-error class="mt-2" :messages="$errors->get('sku')" />
                            </div>

                            <div>
                                <x-input-label for="price" :value="__('Price (₹)')" />
                                <x-text-input id="price" name="price" type="number" step="0.01" min="0.01" class="mt-1 block w-full" :value="old('price')" required placeholder="45000.00" />
                                <x-input-error class="mt-2" :messages="$errors->get('price')" />
                            </div>

                            <div>
                                <x-input-label for="stock" :value="__('Available Stock Quantity')" />
                                <x-text-input id="stock" name="stock" type="number" min="0" class="mt-1 block w-full" :value="old('stock', 1)" required />
                            ...
                            <div>
                                ...
                                <select id="metal" name="metal" x-model="selectedMetal" @change="onMetalChange()" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-100 focus:ring-amber-500 focus:border-amber-500 transition-colors">
                                    ...
                                    @foreach ($metals as $metal)
                                        <option value="{{ $metal->name }}" {{ old('metal') === $metal->name ? 'selected' : '' }}>
                                            {{ $metal->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="colour" name="colour" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-100 focus:ring-amber-500 focus:border-amber-500 transition-colors">
                                    ...
                                    @foreach ($colours as $colour)
                                        <option value="{{ $colour->name }}" {{ old('colour') === $colour->name ? 'selected' : '' }}>
                                            {{ $colour->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <select id="size" name="size" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-100 focus:ring-amber-500 focus:border-amber-500 transition-colors">
                                    ...
                                    @foreach ($sizes as $size)
                                        <option value="{{ $size->value }}" {{ old('size') === $size->value ? 'selected' : '' }}>
                                            {{ $size->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="weight" name="weight" type="number" step="0.001" min="0.001" class="mt-1 block w-full" :value="old('weight')" placeholder="e.g. 4.500" />
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="mt-1 block w-full rounded-xl border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-100 focus:ring-amber-500 focus:border-amber-500 transition-colors">
                                    ...
                                    <option value="inactive" {{ old('status', 'inactive') === 'inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                                </select>
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ __('Add diamonds, rubies, emeralds, or other gems set in this piece.') }}</p>
                            </div>
    ...
    <script>
        function variantCreateForm() {
            ...
            const initialMetal = @json(old('metal', ''));
            const initialPurity = @json(old('purity', ''));
