...
<div {{ $attributes->merge(['class' => 'bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-12 text-center space-y-3 shadow-sm transition-colors']) }}>
    ...
    <h4 class="font-extrabold text-base text-slate-900 dark:text-slate-100">{{ __($title) }}</h4>
    @if ($description)
        <p class="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed">{{ __($description) }}</p>
    @endif
